...
const server = http.createServer((req, res) => {
    ...
    // Static files
    const staticDirs = ['gestao_salas', 'ia_relatorios', 'painel_institucional', 'reservas_emprestimos', 'inventario', 'ocorrencias', 'js', 'data', 'logs'];
    for (const dir of staticDirs) {
        if (url.startsWith('/' + dir + '/') || url === '/' + dir) {
            let file = url === '/' + dir ? '/' + dir + '/code.html' : url;
            const filePath = path.join(__dirname, dir, path.basename(file));
